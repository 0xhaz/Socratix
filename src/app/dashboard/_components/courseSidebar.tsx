...
export function CourseSidebar({ course }: courseSidebarProps) {
  ...
  const approvedByName = course.approvedBy?.name ?? course.author.name;
  const approvedAt = course.approvedAt ?? null;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className=" pb-4 pr-4 border-b border-border">
        ...
        <div className="mb-3">
          <Badge variant="outline" className="max-w-full justify-start whitespace-normal text-left leading-4">
            Approved by {approvedByName}{approvedAt ? `, ${new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(approvedAt)}` : ""}
          </Badge>
        </div>
