...
export default async function SlugPage({ params }: { params: Params }) {
  ...
  // First lesson (chapters & lessons are ordered by position asc) — the course
  // overview route is a stub, so we send students straight into this lesson.
  const firstLessonId = course.chapters[0]?.lessons[0]?.id;
  const courseEntryHref = firstLessonId
    ? `/dashboard/enrolled-courses/${slug}/${firstLessonId}`
    : `/dashboard/enrolled-courses/${slug}`;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-6">
      ...
      <div className="order-1 lg:col-span-2 space-y-6">
        ...
        <div className="relative aspect-video w-full overflow-hidden rounded-t-xl shadow-lg">
          <Image
            src={`/api/s3/file?key=${encodeURIComponent(course.fileKey)}`}
            alt={course.title}
            fill
        ...
        <div className="sticky top-22">
          <Card >
            <CardContent>
              ...
              {isEnrolled ? (
                <Link
                  href={courseEntryHref}
                  className={buttonVariants({ variant: "default" }) + " w-full"}
                >
                  Watch Course
                </Link>
              ) : (
                <EnrollmentButton courseId={course.id} entryHref={courseEntryHref} />
              )}
